...
# Perform a paired ANOVA separately for each gender
anova_res_F <- aov(count ~ visit + Error(patient/visit), data = gene_counts_long %>% filter(sex == "F"))
anova_res_M <- aov(count ~ visit + Error(patient/visit), data = gene_counts_long %>% filter(sex == "M"))

# Extract ANOVA p-values
anova_pval_F <- summary(anova_res_F)[[2]][[1]][["Pr(>F)"]][1]
anova_pval_M <- summary(anova_res_M)[[2]][[1]][["Pr(>F)"]][1]

# Perform paired Wilcoxon tests for each sex
wilcox_res <- list(
  visit1_vs_visit2_F = wilcox.test(
    x = gene_counts_long %>% filter(visit == "visit_1", sex == "F") %>% pull(count),
    y = gene_counts_long %>% filter(visit == "visit_2", sex == "F") %>% pull(count),
    paired = TRUE
  ),
  visit1_vs_visit3_F = wilcox.test(
    x = gene_counts_long %>% filter(visit == "visit_1", sex == "F") %>% pull(count),
    y = gene_counts_long %>% filter(visit == "visit_3", sex == "F") %>% pull(count),
    paired = TRUE
  ),
  visit2_vs_visit3_F = wilcox.test(
    x = gene_counts_long %>% filter(visit == "visit_2", sex == "F") %>% pull(count),
    y = gene_counts_long %>% filter(visit == "visit_3", sex == "F") %>% pull(count),
    paired = TRUE
  ),
  visit1_vs_visit2_M = wilcox.test(
    x = gene_counts_long %>% filter(visit == "visit_1", sex == "M") %>% pull(count),
    y = gene_counts_long %>% filter(visit == "visit_2", sex == "M") %>% pull(count),
    paired = TRUE
  ),
  visit1_vs_visit3_M = wilcox.test(
    x = gene_counts_long %>% filter(visit == "visit_1", sex == "M") %>% pull(count),
    y = gene_counts_long %>% filter(visit == "visit_3", sex == "M") %>% pull(count),
    paired = TRUE
  ),
  visit2_vs_visit3_M = wilcox.test(
    x = gene_counts_long %>% filter(visit == "visit_2", sex == "M") %>% pull(count),
    y = gene_counts_long %>% filter(visit == "visit_3", sex == "M") %>% pull(count),
    paired = TRUE
  )
)

wilcox_pvals <- sapply(wilcox_res, function(x) x$p.value)
...
plot <- ggplot(gene_counts_long, aes(x = interaction(visit, sex), y = count, fill = visit)) +
  ...
  #geom_line(aes(group = Patient), color = "grey", size = 0.3, linetype = "solid", alpha = 0.6) +  # Connect paired values with thin lines
  labs(title = gene_of_interest, x = "Visit and Sex", y = "Gene Count") +
  theme_minimal() +
  geom_signif(
    comparisons = list(
...
# Add ANOVA p-values to the plot
plot <- plot + annotate("text", x = 1, y = max(gene_counts_long$count), label = paste("ANOVA F p-value:", format(anova_pval_F, digits = 3)), hjust = 0, vjust = 1, size = 3)
plot <- plot + annotate("text", x = 4, y = max(gene_counts_long$count), label = paste("ANOVA M p-value:", format(anova_pval_M, digits = 3)), hjust = 0, vjust = 1, size = 3)
...
mat.df2 %>% 
  ...
  lapply(\(vv) {
    ...
    stat.test <- plot.dat %>%
      group_by(bin, sex) %>%
      wilcox_test(mm~sex_visit, paired = T)
...
We provide lists with filtered count matrices per visit.
