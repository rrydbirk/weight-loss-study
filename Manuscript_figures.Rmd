...
```{r, fig.width = 6, fig.height = 4}
tmp <- con.integration$clusters$loft_transfer$groups %>%
  renameAnnotation("Endothelial cells", "ECs") %>%
  renameAnnotation("FAPs", "ASPCs") %>% 
  renameAnnotation("Lymphoid immune cells", "Lymphoid ICs") %>%
  renameAnnotation(c("Tissue-resident macrophages", "Myeloid immune cells"), "Myeloid ICs") %>% 
...
df.plot <- tmp.prop %>% 
  ...
  mutate(anno = factor(anno) %>% 
           factor(levels = levels(.)[c(1, 2, 5, 8, 3, 7, 6, 4)]) %>% 
           renameAnnotation("Lymphatic endothelial cells", "LECs"))
...
First, we integrate adipocytes from both studies. This is not run here
